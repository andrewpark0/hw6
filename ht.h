...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), mIndex_(0), resizeAlpha_(resizeAlpha), deletedCount(0), totalCount(0)
{
...
{
    return totalCount - deletedCount == 0;
}
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    while(static_cast<double>(totalCount) / CAPACITIES[mIndex_] >= resizeAlpha_){
        resize();
    }

    HASH_INDEX_T index = probe(p.first);
    if(index == npos){
        throw std::logic_error("No available position found for insertion.");
    }
...
{
    if(CAPACITIES[mIndex_] == 1685759167){
        std::cerr << "Maximum capacity reached, cannot resize further." << std::endl;
        return;
    }
    ...
    for(size_t i = 0; i < table_.size(); ++i){
        ...
        if(item && !item->deleted){
          HASH_INDEX_T newIndex = hash_(item->item.first) % newSize;
